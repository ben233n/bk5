...
if ($(window).width() < 1200) {
  ...
  .onSnapshot(function (snapshot) {
    ...
    snapshot.forEach(function (doc) {
      ...
      // 設置隨機位置
      randomDiv.css({
        top: Math.random() * $(document).height() - 160 + 'px',  // 隨機垂直位置（整個頁面高度）
        left: Math.random() * $(document).width() + 'px',  // 隨機水平位置
      });
    ...
    $(document).ready(function() {
      ...
      function moveDiv() {
        ...
        // 檢查是否碰到邊界，並反彈
        if (newTop <= 0 || newTop + divHeight >=footerTop + footerHeight) {
          directionY = Math.random() > 0.5 ? 1 : -1;  // 隨機改變 Y 軸方向
          
          rotationAngle = Math.random() * 360;  // 隨機旋轉角度
        }
    
        if (newLeft <= 0 || newLeft + divWidth >= windowWidth) {
